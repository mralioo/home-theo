...
def _req(text, **kw):
    return InboundRequest(request_id=kw.pop("rid", "t1"), channel=Channel.phone,
                          raw_text=text, **kw)


def test_low_cost_heating_auto_resolves():
    r = handle_request(_req("The heating is not working, please help.",
                            property_hint="Musterstrasse 12", rid="r-heat"))
...
def test_financial_escalates_to_human():
    r = handle_request(_req("Why was this invoice charged to me?",
                            property_hint="Musterstrasse 12", rid="r-fin"))
    assert r.decision == Decision.escalate_human
    assert r.vendor_plan is None


def test_angry_sentiment_escalates():
    r = handle_request(_req("This is the third time! Unacceptable!!!",
                            property_hint="Musterstrasse 12", rid="r-angry"))
    assert r.decision == Decision.escalate_human


def test_emergency_detected():
    r = handle_request(_req("There is flooding in the basement!",
                            property_hint="Musterstrasse 12", rid="r-flood"))
